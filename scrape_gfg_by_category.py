...
def write_to_html(url, article_no, file_name):
...
def get_all_links(url, file_name):
    ...
    try:
        ...
        print('Some error in the article', article_no)
    if len(s)>0:
        soup = str(s[0])
        soup = BeautifulSoup(soup)
        for link in soup.findAll('a', href=True):
            try:
                write_to_html(str(link['href']), article_no, file_name)
                article_no += 1
                print(link['href'])
            except:
